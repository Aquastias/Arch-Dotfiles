...
- [ ] A `[theme.templates.user.kitty]` declares a stowed static Mustache input
      ...
      `terminal_*`/Material roles into kitty color, output to the generated theme
      file `kitty.conf` includes.
...
- [ ] Font family is the installed Nerd variant so p10k glyphs render; automatic
      bold; reviewed size.
- [ ] Reviewed knobs applied: `LS_COLORS` env pass-through part-file dropped from
      the include list; tab bar hidden for a single tab; titlebar follows the
      palette; config auto-reload pinned on; active-window border follows the
      accent; transparent background and beam cursor kept.
