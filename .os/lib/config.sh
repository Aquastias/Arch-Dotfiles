...
detect_mode() {
    ...
    local cfg_mode; cfg_mode="$(cfgo '.mode')"
    ...
    local single_disk; single_disk="$(cfgo '.disk')"
    local os_cnt; os_cnt="$(jq '.os_pool.disks | length // 0' "$CONFIG_FILE")"
...
validate_config() {
    ...
    if [[ "$INSTALL_MODE" == "single" ]]; then
        ...
        local cnt; cnt="$(jq '.os_pool.disks | length' "$CONFIG_FILE")"
        ...
        for (( i=0; i<sg; i++ )); do
            local gname; gname="$(cfg ".storage_groups[$i].name")"
            local gdc;   gdc="$(jq ".storage_groups[$i].disks | length" "$CONFIG_FILE")"
...
print_summary() {
    ...
    if [[ "$INSTALL_MODE" == "single" ]]; then
        local d; d="$(cfgo '.disk')"
        local sz; sz="$(lsblk -dno SIZE "$d" 2>/dev/null || echo '?')"
        ...
        if (( sg > 0 )) || $has_left; then
            ...
            for (( i=0; i<sg; i++ )); do
                local gn; gn="$(cfg ".storage_groups[$i].name")"
                local gm; gm="$(cfg ".storage_groups[$i].mount")"
    ...
    # Packages
    local extras; extras="$(jq -r '(.packages.extra // []) | join(", ")' "$CONFIG_FILE")"
    local cli;    cli="$(jq -r '(.packages.groups.cli // []) | join(", ")' "$CONFIG_FILE")"
    local dev;    dev="$(jq -r '(.packages.groups.dev // []) | join(", ")' "$CONFIG_FILE")"
    local gui;    gui="$(jq -r '(.packages.groups.gui // []) | join(", ")' "$CONFIG_FILE")"
    echo ""
    ...
    # Post-install
    local kde;      kde="$(cfgo '.post_install.kde')";           kde="${kde:-false}"
    local backup;   backup="$(cfgo '.post_install.backup')";     backup="${backup:-false}"
    local security; security="$(cfgo '.post_install.security')"; security="${security:-false}"
    echo ""
    ...
    echo ""
    local enc; enc="$(cfgo '.options.encryption')"; enc="${enc:-false}"
    local swap; swap="$(cfgo '.options.swap')"; swap="${swap:-true}"
    printf "  %-16s %s\n" "Hostname:"   "$(cfg '.system.hostname')"
    printf "  %-16s %s\n" "User:"       "$(cfg '.system.username')"
