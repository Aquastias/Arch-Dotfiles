...
# ZPOOL_VDEV_NAME_PATH=YES makes grub-probe resolve the pool's vdevs by full
# /dev path, so grub-mkconfig succeeds on a ZFS root without grub-libzfs
# (OpenZFS-documented method; stock core/grub already ships the zfs module).
