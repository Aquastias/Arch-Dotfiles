...
# derived System Program. The ENUM generalisation of the printing/bluetooth
...
# System Program of the same name; `none` derives nothing.
#
...
# derives the program list, and power_inject folds it into system_programs — so
...
#   power_programs          <config-json> → derived System Program, one/line
...
# power_programs <config-json> — the toggle-derived System Program for the
...
# power_inject <config-json> — the config with the derived System Program folded
# into .system_programs (order-preserving, idempotent). A no-op when power
...
# power_owned_programs — every System Program the Power enum can own, regardless
