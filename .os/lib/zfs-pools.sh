...
install_zfs_tools_if_needed() {
    ...
    if ! grep -q '\[archzfs\]' /etc/pacman.conf; then
        local kf=/tmp/archzfs-fallback.gpg
        curl -fsSL https://archzfs.com/archzfs.gpg -o "$kf" \
            || error "Failed to download archzfs GPG key."
        pacman-key --add "$kf"
        local fp
        fp="$(gpg --with-colons --import-options show-only --import "$kf" 2>/dev/null \
...
_zpool_create() {
    ...
    local pool_name="$1" ashift="$2"; shift 2
    local vdev_spec="$*"
...
_create_os_datasets() {
    ...
    local swap_on; swap_on="$(cfgo '.options.swap')"; swap_on="${swap_on:-true}"
    local cfg_swap_sz; cfg_swap_sz="$(cfgo '.options.swap_size')"
    local swap_arg
