...
check_disk_space() {
    # The Arch ISO mounts / as tmpfs (default: half of RAM).
    # zfs-dkms + linux-headers + build tools need ~600–900 MB.
    # zfs-linux (pre-built) needs ~40–80 MB. Either way we need headroom.
    section "Checking Available RAM/tmpfs Space"
    local avail_mb
    avail_mb="$(df -m / | awk 'NR==2{print $4}')"
    info "Available space on live root (tmpfs): ${avail_mb} MB"

    if (( avail_mb < 400 )); then
        error "Only ${avail_mb} MB free on tmpfs. Need at least 400 MB.
  This usually means your machine has < 1 GB RAM.
  Try adding 'cow_spacesize=2G' to the kernel cmdline when booting the ISO,
  or use a machine with more RAM (2+ GB recommended)."
    elif (( avail_mb < 800 )); then
        warn "Only ${avail_mb} MB free. DKMS build may fail if pre-built module is unavailable."
        warn "Pre-built module (zfs-linux) needs ~80 MB; DKMS build needs ~800 MB."
    else
        info "Space OK for both pre-built and DKMS paths."
    fi
}
