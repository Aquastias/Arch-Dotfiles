...
# Drives the Guided Installer headlessly: resolve the install disk in-guest via
# the Pre-Install Picker, write a replay answers file, run install.sh --guided.
# set -e is intentionally OFF (see the non-guided renderer for the rationale).
output: {all: '| tee -a /dev/ttyS0'}
runcmd:
  - |
    set +e
