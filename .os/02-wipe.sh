...
_on_error() { echo -e "\n${RED}[ERROR]${NC} Wipe script failed at line $1." >&2; exit 1; }
...
detect_disks() {
    local live_disk; live_disk="$(find_live_disk)"
...
disk_info_table() {
    ...
    for disk in "${disks[@]}"; do
        ...
        if   [[ "$tran" == "nvme" ]];  then dtype="NVMe"
        elif [[ "$rota" == "0"    ]];  then dtype="SSD"
        elif [[ "$rota" == "1"    ]];  then dtype="HDD"
        else dtype="?"; fi
...
select_disks() {
    ...
    [[ -z "$excl" ]] && { info "All disks selected for wiping."; return; }
...
print_summary() {
    ...
    for disk in "${DISKS_TO_WIPE[@]}"; do
        local log="/tmp/wipe-$(basename "$disk").log"
        local last; last="$(tail -1 "$log" 2>/dev/null || echo 'no log')"
...
main() {
    ...
    (( ${#DISKS_TO_WIPE[@]} > 0 )) || { info "No disks selected. Nothing to do."; exit 0; }
