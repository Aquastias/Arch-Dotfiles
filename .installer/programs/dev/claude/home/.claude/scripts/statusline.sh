...
# Severity colour: RED at/above crit, YELLOW at/above warn, else GREEN. Used by
# the context bar and the 5h meter (both: higher = worse).
pick3() {
  if   [ "$1" -ge "$3" ]; then printf '%b' "$RED"
  elif [ "$1" -ge "$2" ]; then printf '%b' "$YELLOW"
...
if [ "${WINDOW:-0}" -gt 0 ] 2>/dev/null; then
  _t="$(fmt_tokens "$USED")/$(fmt_tokens "$WINDOW")"
  TOKENS_SEG=" ${DIM}· ${_t} tok${RESET}"
fi

# Cache-hit rate — native prompt_cache.hit_ratio (replaces the old transcript
# tail). Green efficient, red poor (higher = better, so not pick3).
CACHE_SEG=""
...
# Sandbox status.
SB_ON="$(jq -r '.sandbox.enabled // false' \
  "$HOME/.claude/settings.json" 2>/dev/null)"
if [ "${SANDBOX_RUNTIME:-}" = "1" ] || [ "$SB_ON" = "true" ]; then
...
BILLING=$(grep -oE '"billingType" *: *"[^"]*"' "$HOME/.claude.json" \
  2>/dev/null | head -1 | cut -d'"' -f4)
PLAN=""; CREDS="$HOME/.claude/.credentials.json"
if [ -f "$CREDS" ]; then
  _oauth=$(jq -r '.claudeAiOauth
    | "\(.subscriptionType // "-") \(.rateLimitTier // "-")"' \
    "$CREDS" 2>/dev/null)
  read -r SUBTYPE RLTIER <<<"$_oauth"
  case "$SUBTYPE" in
    max) case "$RLTIER" in
...
if [ -z "$PLAN" ]; then
  case "$BILLING" in
    *subscription*) PLAN="Subscription" ;; "") ;; *) PLAN="API" ;;
  esac
fi
PLAN_SEG=""
[ -n "$PLAN" ] && PLAN_SEG="${SEP}${CYAN}${E_PLAN} ${PLAN}${RESET}"
...
if command -v ccusage >/dev/null 2>&1; then
  ...
  if [ -s "$CCU_CACHE" ]; then
    CCU_COST=$(jq -r \
      'first(.blocks[]? | select(.isActive==true) | .costUSD) // empty' \
      "$CCU_CACHE" 2>/dev/null)
    [ -n "$CCU_COST" ] &&
      CCU_SEG="${SEP}${E_BURN} $(printf '$%.2f' "$CCU_COST")"
  fi
fi

# Third slot on line 2: on a subscription the 5-hour meter (+ ccusage burn); on
# an API plan the per-session $cost.
THIRD_SEG=""
if [ "$IS_API" = 1 ]; then
  THIRD_SEG="${SEP}${YELLOW}$(printf '$%.2f' "$COST")${RESET}"
elif [ -n "$R5_PCT" ]; then
  R5C=$(pick3 "$R5_PCT" 50 80)
  RESET_TXT=""
  if [ -n "$R5_RESET" ]; then
    REM=$(( R5_RESET - $(date +%s) ))
...
LINES_SEG=""
if [ "${LINES_ADD:-0}" -gt 0 ] 2>/dev/null ||
   [ "${LINES_DEL:-0}" -gt 0 ] 2>/dev/null; then
...
L1="${CYAN}${MODEL_SEG}${RESET} ${E_DIR} ${DIR##*/}${BRANCH_SEG}"
L1+="${SEP}${SANDBOX_SEG}${PLAN_SEG}"
L2="${BAR_COLOR}${BAR}${RESET} ${PCT}%${TOKENS_SEG}${CACHE_SEG}${THIRD_SEG}"
L2+="${SEP}${E_CLK} ${MINS}m ${SECS}s${LINES_SEG}"
printf '%b\n' "$L1"
printf '%b\n' "$L2"
