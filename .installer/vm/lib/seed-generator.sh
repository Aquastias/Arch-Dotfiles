...
_seed_generator_session_firstboot_block() {
  ...
  local fx=/root/dotfiles/.installer/vm/fixtures/desktop-verify
  cat <<BLOCK
    if [ "\$rc" -eq 0 ]; then
...
_seed_generator_multi_firstboot_block() {
  ...
  cat <<BLOCK
    if [ "\$rc" -eq 0 ]; then
      ...
      zfs mount rpool/ROOT/arch || true
$(_seed_generator_esp_serial_lines)
      install -Dm644 /root/dotfiles/.installer/vm/lib/vm-pool-verify.sh "/mnt${lib}"
...
_seed_generator_esp_resilience_firstboot_block() {
  ...
  cat <<BLOCK
    if [ "\$rc" -eq 0 ]; then
      ...
      zfs mount rpool/ROOT/arch || true
$(_seed_generator_esp_serial_lines)
      install -Dm644 /root/dotfiles/.installer/vm/lib/esp-resilience-verify.sh "/mnt${lib}"
      mkdir -p /mnt/etc/systemd/system/multi-user.target.wants
...
_seed_generator_render_user_data() {
  ...
  cat <<EOF
#cloud-config
# Generated by .installer/lib/seed-generator.sh — do not edit by hand.
...
EOF
  ...
  cat <<EOF
#cloud-config
# Generated by .installer/lib/seed-generator.sh (guided) — do not edit by hand.
