...
### Storage Group
A vdev (or set of per-disk vdevs) folded into the single Combined Data Pool in multi-disk mode. Declared in `storage_groups[]` in the Install Config (`name`, `disks`, `mount`, optional `topology`/`ashift`). Each group surfaces as datasets under `dpool/DATA/<name>`; all groups share one pool and therefore one failure domain. Use a Storage Group when you want several disks pooled together (with redundancy) under one name. Contrast Standalone Data Pool.
